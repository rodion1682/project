<template>
  <BaseModal
    :show="props.show"
    modal-container-class="withdraw-modal__inner"
    @close="close"
  >
    <div class="withdraw-modal">
      <h2 class="withdraw-modal__title _h3">
        {{ $t('Withdraw') }}
      </h2>

      <div class="withdraw-modal__summary">
        <div class="withdraw-modal__summary-row">
          <span class="withdraw-modal__summary-label">
            {{ $t('Balance') }}:
          </span>

          <span class="withdraw-modal__total">
            {{ formattedBalance }}
          </span>
        </div>

        <div class="withdraw-modal__summary-divider" />

        <div class="withdraw-modal__summary-row">
          <span class="withdraw-modal__summary-secondary-label">
            {{ $t('Currency') }}:
          </span>

          <span class="withdraw-modal__summary-secondary-value">
            {{ currencyCode }}
          </span>
        </div>
      </div>
      ...
      <div
        ...
        class="withdraw-modal__error withdraw-modal__error_general"
        role="alert"
      >
        {{ formErrors.general }}
      </div>

      <form class="withdraw-modal__form" novalidate @submit.prevent="handlePay">
        <div class="withdraw-modal__section">
          <p class="withdraw-modal__section-title">
            {{ $t('Personal information') }}
          </p>

          <div class="withdraw-modal__fields">
            <div class="withdraw-modal__field">
              <BaseInput
                v-model="topupStore.checkoutFormData.name"
                class="withdraw-modal__input"
        ...
        <div class="withdraw-modal__section withdraw-modal__section_payment">
          <p class="withdraw-modal__section-title">
          ...
          <div class="withdraw-modal__fields">
            ...
            <div class="withdraw-modal__field-row">
              <div class="withdraw-modal__field">
                <BaseInput
              ...
              <div
                class="withdraw-modal__field withdraw-modal__field_post-code"
              >
                <BaseInput
        ...
        <div class="withdraw-modal__terms">
          ...
          <span
            ...
            class="withdraw-modal__field-error"
          >
...
const currentBalance = computed(() => {
  ...
  return normalizeNumber(
    profile.balance ?? profile.wallet_balance ?? profile.available_balance ?? 0,
  )
})
...
const formattedBalance = computed(() => {
  try {
    ...
    return `${currencySymbol.value}` + currentBalance.value.toFixed(2)
  }
})
...
const isFormValid = computed(() => {
  ...
  return Boolean(
    ...
    normalizeNumber(amount.value) > 0 &&
...
const validateForm = () => {
  ...
  const normalizedAmount = normalizeNumber(amount.value)

  if (normalizedAmount <= 0) {
    errors.amount = 'Please enter a valid amount'
  }

  if (currentBalance.value > 0 && normalizedAmount > currentBalance.value) {
...
const scrollToFirstError = async () => {
  ...
  document
    .querySelector(
      '.withdraw-modal__error, .base-input__error, .base-select__error',
    )
    ?.scrollIntoView({
      behavior: 'smooth',
      block: 'center',
...
const handlePay = async () => {
  ...
  try {
    ...
    const checkoutData = {
      ...
      amount: normalizeNumber(amount.value),

      currency: currencyCode.value,
    }

    const response = await offerFlowStore.requestWithdraw(checkoutData)
...
const initializeModal = async () => {
  ...
  try {
    ...
    console.error('Failed to initialize withdraw modal:', error)

    const message =
      error.response?.data?.message ||
      error.message ||
      'Unable to prepare the withdraw form.'
...
.withdraw-modal {
  ...
  &__summary-secondary-label {
    color: var(--secondary-color);
    font-size: 14px;
  }

  &__summary-secondary-value {
    color: var(--primary-color);
    font-size: 15px;
    font-weight: 600;
    line-height: 140%;
